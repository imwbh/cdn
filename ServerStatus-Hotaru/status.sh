...
Download_Server_Status_server() {
  cd "/tmp" || exit 1
  wget -N --no-check-certificate "https://cdn.jsdelivr.net/gh/imwbh/cdn/ServerStatus-Hotaru/ServerStatus-Hotaru-master.zip"
  [[ ! -e "ServerStatus-Hotaru-master.zip" ]] && echo -e "${Error} ServerStatus 服务端下载失败 !" && exit 1
  unzip master.zip
  rm -rf master.zip
...
Download_Server_Status_client() {
  cd "/tmp" || exit 1
  wget -N --no-check-certificate "https://cdn.jsdelivr.net/gh/CokeMine/ServerStatus-Hotaru@master/clients/status-client.py"
...
Service_Server_Status_server() {
  if [[ ${release} == "centos" ]]; then
    if ! wget --no-check-certificate "https://cdn.jsdelivr.net/gh/CokeMine/ServerStatus-Hotaru@master/service/server_status_server_centos" -O /etc/init.d/status-server; then
    ...
    if ! wget --no-check-certificate "https://cdn.jsdelivr.net/gh/CokeMine/ServerStatus-Hotaru@master/service/server_status_server_debian" -O /etc/init.d/status-server; then
      echo -e "${Error} ServerStatus 服务端服务管理脚本下载失败 !" && exit 1
    fi
    chmod +x /etc/init.d/status-server
...
Service_Server_Status_client() {
  if [[ ${release} == "centos" ]]; then
    if ! wget --no-check-certificate "https://cdn.jsdelivr.net/gh/CokeMine/ServerStatus-Hotaru@master/service/server_status_client_centos" -O /etc/init.d/status-client; then
    ...
    if ! wget --no-check-certificate "https://cdn.jsdelivr.net/gh/CokeMine/ServerStatus-Hotaru@master/service/server_status_client_debian" -O /etc/init.d/status-client; then
      echo -e "${Error} ServerStatus 客户端服务管理脚本下载失败 !" && exit 1
    fi
    chmod +x /etc/init.d/status-client
...
Install_jq() {
  if [[ ! -e ${jq_file} ]]; then
    if [[ ${bit} == "x86_64" ]]; then  
      wget --no-check-certificate "https://cdn.jsdelivr.net/gh/imwbh/cdn/ServerStatus-Hotaru/jq-linux64" -O ${jq_file}
    else
      wget --no-check-certificate "https://cdn.jsdelivr.net/gh/imwbh/cdn/ServerStatus-Hotaru/jq-linux32" -O ${jq_file}
    fi
  ...
  if [[ ! -e ${region_json} ]]; then
    wget --no-check-certificate "https://cdn.jsdelivr.net/gh/michaelwittig/node-i18n-iso-countries/langs/zh.json" -O ${region_json}
...
Install_caddy() {
  ...
  if [[ "${caddy_yn}" == [Yy] ]]; then
    ...
    if [[ ! -e "/usr/local/caddy/caddy" ]]; then
      wget -N --no-check-certificate https://cdn.jsdelivr.net/gh/CokeMine/ServerStatus-Hotaru@master/caddy/caddy_install.sh
...
Update_Shell() {
  sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://cdn.jsdelivr.net/gh/CokeMine/ServerStatus-Hotaru@master/status.sh" | grep 'sh_ver="' | awk -F "=" '{print $NF}' | sed 's/\"//g' | head -1)
  ...
  wget -N --no-check-certificate "https://cdn.jsdelivr.net/gh/CokeMine/ServerStatus-Hotaru@master/status.sh" && chmod +x status.sh
